   ...
   private:
    ...
    void scan_callback(const sensor_msgs::msg::LaserScan::ConstSharedPtr scan_msg) {
        ...
        for (std::size_t i = 0; i < scan_msg->ranges.size(); i++) {
            ...
            double threshold_slow = 2;  // To be tuned in real vehicle
            if (r / std::max(this->speed * std::cos(scan_msg->angle_min + (double)i * scan_msg->angle_increment), 0.001) < threshold_slow && r / std::max(this->speed * std::cos(scan_msg->angle_min + (double)i * scan_msg->angle_increment), 0.001) > threshold) {
               drive_msg.drive.speed = drive_msg.drive.speed /2;
               RCLCPP_INFO(this->get_logger(), "Slow down'%f'", this->speed);  // Output to log;
               break;
            }
