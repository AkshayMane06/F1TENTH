...
class SafetyNode(Node):
    ...
    def scan_callback(self, scan_msg):
        ...
        for idx, r in enumerate(scan_msg.ranges):
            ...
            threshold_slow = 5 
	    if r / max(self.speed * np.cos(scan_msg.angle_min + idx * scan_msg.angle_increment), 0.001) <= threshold_slow: 
		self.get_logger().info("Slowing down at speed {}".format(self.speed)) # Output to Log
        ...
        if emergency_breaking:
            ...
            self.get_logger().info("emergency brake engaged at a speed {}".format(self.speed)) # Output to Log
